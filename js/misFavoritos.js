...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!usuarioId) {
    container.innerHTML = `<div class="alert alert-warning">Debés iniciar sesión para ver tus favoritos.</div>`;
    return;
  }
  if (!token) {
  ...
  try {
    const response = await fetch(`${URL_API}/usuarios/${usuarioId}/favoritos`,{
      method: 'GET',
      headers: {
    ...
    if (!Array.isArray(publicaciones) || publicaciones.length === 0) {
      container.innerHTML = `<div class="alert alert-info">No tenes publicaciones favoritas .</div>`;
      return;
    }
