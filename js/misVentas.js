...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (ventas.length === 0) {
      contenedor.innerHTML = `<div class="alert alert-info">No tenés ventas aún.</div>`;
      return;
    }
    ...
    for (const venta of ventas) {
      ...
      let comprador = null;
      const vendResp = await fetch(`${URL_API}/usuarios/${venta.idComprador}`);
      comprador = vendResp.ok ? await vendResp.json() : null;
      ...
      html += `
        ...
        <hr/>
        <h6>Comprador</h6>
        <p>${comprador ? comprador.nombre + ' ' + comprador.apellido : 'No disponible'}</p>
